...
const pages = ref([])
const canvasRefs = []
const regions = ref(props.regionsFromServer)
...
const currentImagePath = ref(props.documentPath);

const imageUrls = [currentImagePath.value] // Or just one image
pages.value = imageUrls

const renderImageDocument = async () => {

    for (let i = 0; i < imageUrls.length; i++) {
        await renderImage(imageUrls[i], i)
    }
}

import { nextTick } from 'vue'

const renderImage = async (imageUrl, index) => {
    await nextTick() // ✅ Wait for canvas to be in DOM

    const canvas = canvasRefs[index]
    if (!canvas) {
        console.warn(`Canvas at index ${index} is not ready yet.`)
        return
    }
...
const onMouseUp = () => {
    ...
    if (tempRegion.width < 0) {
        tempRegion.x = Math.abs(tempRegion.x + tempRegion.width)
        tempRegion.width = Math.abs(tempRegion.width)
    }
    if (tempRegion.height < 0) {
        tempRegion.y = Math.abs(tempRegion.y + tempRegion.height)
...
const openFormDialog = () => {
    selectedRegion.value.type = (selectedRegion.value.type && selectedRegion.value.type != "") ? selectedRegion.value.type : "multiple_choice"
